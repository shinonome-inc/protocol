...
contract DeployZeroEx is Test {
    ...
    function logDeployedZeroEx() public {
        ...
        emit log_named_address(
            "BatchFillNativeOrdersFeature", address(ZERO_EX_DEPLOYED.features.batchFillNativeOrdersFeature)
            );
        ...
        emit log_named_address(
            "AffiliateFeeTransformer", address(ZERO_EX_DEPLOYED.transformers.affiliateFeeTransformer)
            );
    ...
    function deployZeroEx() public returns (ZeroExDeployed memory) {
        ...
        ZERO_EX_DEPLOYED.features.erc1155OrdersFeature =
            new ERC1155OrdersFeature(address(ZERO_EX), ZERO_EX_DEPLOYED.weth);
        ...
        IZERO_EX.migrate(
            ...
            abi.encodeWithSelector(
                TransformERC20Feature.migrate.selector, address(ZERO_EX_DEPLOYED.transformerDeployer)
            ),
            address(this)
        );
        IZERO_EX.migrate(
        ...
        ZERO_EX_DEPLOYED.transformers.payTakerTransformer =
            PayTakerTransformer(ZERO_EX_DEPLOYED.transformerDeployer.deploy(type(PayTakerTransformer).creationCode));
