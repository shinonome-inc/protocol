...
contract WooPPSampler is SamplerUtils, ApproximateBuys {
    function query(uint256 amountIn, address tokenIn, address tokenOut, address pool)
        internal
        view
    ...
    function _sampleSellForApproximateBuyFromWoofi(
        ...
        (address _pool, address _takerToken, address _makerToken) =
            abi.decode(takerTokenData, (address, address, address));
        (bool success, bytes memory resultData) = address(this).staticcall(
            abi.encodeWithSelector(
                this.sampleSellsFromWooPP.selector, _pool, _takerToken, _makerToken, _toSingleValueArray(sellAmount)
            )
        );
        if (!success) {
            return 0;
