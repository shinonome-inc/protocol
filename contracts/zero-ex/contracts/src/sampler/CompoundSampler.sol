...
contract CompoundSampler is SamplerUtils {
    ...
    function sampleSellsFromCompound(
        ...
        if (address(makerToken) == address(cToken)) {
            ...
            for (uint256 i = 0; i < numSamples; i++) {
                makerTokenAmounts[i] =
                    (takerTokenAmounts[i] * EXCHANGE_RATE_SCALE * 10 ** cTokenDecimals) / exchangeRate;
            }
        } else if (address(takerToken) == address(cToken)) {
            // redeem
            for (uint256 i = 0; i < numSamples; i++) {
                makerTokenAmounts[i] =
                    (takerTokenAmounts[i] * exchangeRate) / (EXCHANGE_RATE_SCALE * 10 ** cTokenDecimals);
            }
        }
    }
    ...
    function sampleBuysFromCompound(
        ...
        if (address(makerToken) == address(cToken)) {
            ...
            for (uint256 i = 0; i < numSamples; i++) {
                takerTokenAmounts[i] =
                    (makerTokenAmounts[i] * exchangeRate) / (EXCHANGE_RATE_SCALE * 10 ** cTokenDecimals);
            }
        } else if (address(takerToken) == address(cToken)) {
            // redeem
            for (uint256 i = 0; i < numSamples; i++) {
                takerTokenAmounts[i] =
                    (makerTokenAmounts[i] * EXCHANGE_RATE_SCALE * 10 ** cTokenDecimals) / exchangeRate;
            }
        }
    }
}
