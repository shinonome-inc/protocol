...
contract GMXSampler is SamplerUtils, ApproximateBuys {
    ...
    function sampleSellsFromGMX(IGMX reader, address vault, address[] memory path, uint256[] memory takerTokenAmounts)
        public
        view
        returns (uint256[] memory makerTokenAmounts)
    {
        ...
        for (uint256 i = 0; i < numSamples; i++) {
            ...
            try IGMX(reader).getAmountOut(IVault(vault), path[0], path[1], takerTokenAmounts[i]) returns (
                uint256 amountAfterFees, uint256 feeAmount
            ) {
    ...
    function sampleBuysFromGMX(IGMX reader, address vault, address[] memory path, uint256[] memory makerTokenAmounts)
        public
        view
    ...
    function _sampleSellForApproximateBuyFromGMX(
        ...
        (address _reader, address _vault, address[] memory _path) =
            abi.decode(takerTokenData, (address, address, address[]));

        (bool success, bytes memory resultData) = address(this).staticcall(
            abi.encodeWithSelector(
                this.sampleSellsFromGMX.selector, _reader, _vault, _path, _toSingleValueArray(sellAmount)
            )
        );
        if (!success) {
            return 0;
