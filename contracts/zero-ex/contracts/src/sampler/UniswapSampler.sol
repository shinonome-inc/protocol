...
contract UniswapSampler is SamplerUtils {
    ...
    function sampleSellsFromUniswap(
        ...
        IUniswapExchangeQuotes takerTokenExchange =
            takerToken == address(0) ? IUniswapExchangeQuotes(0) : _getUniswapExchange(router, takerToken);
        IUniswapExchangeQuotes makerTokenExchange =
            makerToken == address(0) ? IUniswapExchangeQuotes(0) : _getUniswapExchange(router, makerToken);
        for (uint256 i = 0; i < numSamples; i++) {
            ...
            if (makerToken == address(0)) {
                ...
                if (ethBought != 0) {
                    (makerTokenAmounts[i], didSucceed) = _callUniswapExchangePriceFunction(
                        address(makerTokenExchange), makerTokenExchange.getEthToTokenInputPrice.selector, ethBought
                    );
                } else {
    ...
    function sampleBuysFromUniswap(
        ...
        for (uint256 i = 0; i < numSamples; i++) {
            ...
            if (makerToken == address(0)) {
                ...
                if (ethSold != 0) {
                    (takerTokenAmounts[i], didSucceed) = _callUniswapExchangePriceFunction(
                        address(takerTokenExchange), takerTokenExchange.getTokenToEthOutputPrice.selector, ethSold
                    );
                } else {
