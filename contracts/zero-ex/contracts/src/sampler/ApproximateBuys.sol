...
contract ApproximateBuys {
        ...
        for (uint256 i = 0; i < makerTokenAmounts.length; i++) {
            ...
            for (uint256 iter = 0; iter < APPROXIMATE_BUY_MAX_ITERATIONS; iter++) {
                ...
                sellAmount = _safeGetPartialAmountCeil(
                    (ONE_HUNDED_PERCENT_BPS + APPROXIMATE_BUY_TARGET_EPSILON_BPS), ONE_HUNDED_PERCENT_BPS, sellAmount
                );
                if (sellAmount == 0) {
                    break;
                }
    ...
    function _safeGetPartialAmountCeil(uint256 numerator, uint256 denominator, uint256 target)
        internal
        view
