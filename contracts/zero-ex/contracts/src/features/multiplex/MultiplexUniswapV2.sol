...
abstract contract MultiplexUniswapV2 is FixinCommon, FixinTokenSpender {
    ...
    function _batchSellUniswapV2External(
        ...
        // Validate tokens
        require(
            tokens.length >= 2 && tokens[0] == address(params.inputToken)
    ...
    function _multiHopSellUniswapV2(
        ...
        // Validate the tokens
        require(
            tokens.length >= 2 && tokens[0] == params.tokens[state.hopIndex]
    ...
    function _sellToUniswapV2(
        ...
        for (uint256 i = 0; i < tokens.length - 1; i++) {
            ...
            (uint256 amount0Out, uint256 amount1Out) =
                inputToken < outputToken ? (uint256(0), outputTokenAmount) : (outputTokenAmount, uint256(0));
            // The Uniswap pair contract will transfer the output tokens to
            // the next pair contract if there is one, otherwise transfer to
            // `recipient`.
            address to =
                i < tokens.length - 2 ? _computeUniswapPairAddress(outputToken, tokens[i + 2], isSushi) : recipient;
            // Execute the swap.
    ...
    // given tokens.
    function _computeUniswapPairAddress(address tokenA, address tokenB, bool isSushi)
        internal
        view
    ...
    function _computeUniswapOutputAmount(
        ...
        (uint256 inputReserve, uint256 outputReserve) =
            inputToken < outputToken ? (reserve0, reserve1) : (reserve1, reserve0);
